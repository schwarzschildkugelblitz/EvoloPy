...
#         numpy.sort(Positions ,key=objf )
#         x_best = Positions[0, :].copy()
#         x_worst = Positions[SearchAgents_no-1, :].copy()
#         #phase 2
#         for i in range(0, SearchAgents_no):
#             # Update the Position of search agents
...
def GROM(objf, lb, ub, dim, SearchAgents_no, Max_iter):
    ...
    for l in range(0, Max_iter):
        ...
        for i in range(0, SearchAgents_no):
            ...
            a = numpy.random.rand()
            if a>=0.5:
                Xnew = Positions[i,:] + (1/2)*numpy.random.rand()*(xb_pos-Xw_pos) # eq 5 in the paper
            if a<=0.5:
                Xnew = (Positions[i,:]-xb_pos)+ (1/2)*numpy.random.rand()*(Positions.mean(0)-Xw_pos) 
            for j in range(dim):
                Xnew[j] = numpy.clip(Xnew[j], lb[j], ub[j])
